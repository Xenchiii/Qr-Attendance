...
<body>
    <div class="main-container">
        <div class="header">
            ...
            <p>Smart attendance tracking made simple</p>
        </div>

        <div class="portal-selector">
            <div class="portal-card active" onclick="switchPortal('student', event)">
                <div class="portal-icon">
                    <i class="fas fa-user-graduate"></i>
                </div>
                <h2>Student Portal</h2>
                <p>Generate your attendance QR code</p>
            </div>

            <div class="portal-card" onclick="switchPortal('teacher', event)">
                <div class="portal-icon">
                    <i class="fas fa-chalkboard-teacher"></i>
                </div>
                <h2>Teacher Portal</h2>
                <p>Manage attendance and students</p>
            </div>
        </div>

        <!-- STUDENT PORTAL -->
        <div id="student-portal" class="portal-content active">
            <div class="card">
                ...
                <div class="form-group">
                    <label class="form-label"><i class="fas fa-hashtag"></i> Student ID</label>
                    <input type="text" class="form-input" id="studentId" placeholder="Enter your student ID">
                </div>

                <div class="form-group">
                    <label class="form-label"><i class="fas fa-book-open"></i> Select Your Class</label>
                    <select class="form-select" id="studentClass">
                        <option value="">Choose your class</option>
                        <option value="SDF04">SDF04 - System Design Fundamentals</option>
                        <option value="IM2">IM2 - Information Management 2</option>
                        <option value="SP2">SP2 - Software Programming 2</option>
                        <option value="SIA1">SIA1 - System Integration & Architecture 1</option>
                    </select>
                </div>
                ...
                <div id="studentQRContainer" class="qr-container hidden">
                    <h4>Your Attendance QR Code</h4>
                    <canvas id="studentqrcode"></canvas>
                    <p class="qr-info">Save this QR code and show it to your teacher for attendance</p>
        ...
        <!-- TEACHER PORTAL -->
        <div id="teacher-portal" class="portal-content">
            <!-- Statistics -->
            ...
            <div class="card">
                ...
                <div class="form-group">
                    <label class="form-label"><i class="fas fa-keyboard"></i> Paste QR Code Data</label>
                    <input type="text" class="form-input" id="manualCode" placeholder="Paste student QR code data here">
                </div>

                <button class="btn btn-primary" onclick="submitManualAttendance()">
            ...
            <div class="card">
                ...
                <div class="form-group">
                    <label class="form-label"><i class="fas fa-filter"></i> Filter by Class</label>
                    <select class="form-select" id="viewClass" onchange="filterStudents()">
    ...
    <script>
        // In-memory data storage (replaces backend API)
        let students = [];
        ...
        function switchPortal(portal, event) {
        ...
        function generateStudentQR() {
            ...
            setTimeout(() => {
                ...
                document.getElementById('studentQRContainer').classList.remove('hidden');
        ...
        function downloadQRCode() {
            ...
            if (canvas) {
                ...
                link.download = `${studentName}-attendance-qr.png`;
        ...
        function printQRCode() {
            ...
            printWindow.document.write(`
                ...
                <body>
                    ...
                    <p>Present this QR code for attendance</p>
                </body>
                </html>
            `);
        ...
        function submitManualAttendance() {
            ...
            try {
                ...
                if (data.class !== className) {
                    showNotification('Student not enrolled in selected class', 'error');
                    return;
                }
        ...
        function markAttendance(studentData) {
            if (!studentData || !studentData.id || !studentData.name || !studentData.class) {
            ...
            setTimeout(() => {
                ...
                const alreadyMarked = attendanceRecords.some(record => 
                    record.student_id === studentData.id && 
                    record.attendance_date === today &&
                    record.class === studentData.class
                );

                if (alreadyMarked) {
                    showNotification('Student already marked present today', 'error');
                    hideLoading();
                    return;
                }
                ...
                showNotification(`✓ ${studentData.name} marked present`, 'success');
                loadData();
                hideLoading();
            }, 500);
        }
        ...
        function renderStudentList() {
            ...
            const classFilter = document.getElementById('viewClass')?.value || 'all';
            ...
            if (!filteredStudents || filteredStudents.length === 0) {
                studentListDiv.innerHTML = `
                    <div class="empty-state">
                        ...
                        <p>Students will appear here after generating QR codes</p>
                    </div>
                `;
                return;
            }
            ...
            studentListDiv.innerHTML = filteredStudents.map(student => {
                ...
                return `
                    <div class="student-item ${isPresent ? '' : 'absent'}">
                        <div class="student-info">
                            <div class="student-name">${student.student_name}</div>
                            <div class="student-details">${student.student_id} • ${student.class}</div>
                        </div>
                        <div class="student-actions">
                            ...
                            ${!isPresent ? `
                                <button class="btn btn-success btn-small" onclick='markStudentAttendance(${JSON.stringify(student).replace(/'/g, "&apos;")})'>
                                    <i class="fas fa-check"></i> Mark
                                </button>
                            ` : ''}
                            <button class="btn btn-danger btn-small" onclick="removeStudent('${student.student_id}')">
        ...
        function removeStudent(studentId) {
            ...
            setTimeout(() => {
                ...
                attendanceRecords = attendanceRecords.filter(r => r.student_id !== studentId);
            ...
            container.innerHTML = todayRecords.map(record => `
                <div class="attendance-item">
                    <div class="student-info">
                        ...
                        <div class="student-details">${record.student_id} • ${record.class}</div>
                    </div>
                    <div class="attendance-time">
                        <i class="fas fa-clock"></i> ${record.attendance_time}
                    </div>
                </div>
            `).join('');
        }
